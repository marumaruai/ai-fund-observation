...
1. **`portfolio/holdings.md`（Private）を更新**
   - 別プラットフォームのGrokへ依頼し、各ファンドの保有株数・現在値・取得単価・評価損益を最新化する。

2. **市場概況を書く**（`reports/YYYY-MM.md`、Private）
   - 別プラットフォームのGrokへ依頼し、直近1ヶ月の市場全体の動き、主要ニュースを**簡潔に**まとめる。
...
4. 一次判断を `reports/YYYY-MM.md` の「## 一次判断」セクションにまとめる(Private)
...
5. **（任意）相互レビュー**（`investing/prompts/peer_review_prompt.md`）
   ...
   - 最終判断は「## 最終判断」セクションに追記する。一次判断のセクションは上書きしない。
...
- 変更時はコミットとPushすること。Pushに失敗することがあるため、Push Originボタンが消えない場合は再度実行し、反映を確認する。
- 「別プラットフォームのGrok」（𝕏上のGrok）は、Grokファンド（投資判断AIとしてのGrok）とは無関係の作業ツールとして利用する。holdings.md更新・市場概況作成の代行のみを行い、投資判断には一切関与しない。